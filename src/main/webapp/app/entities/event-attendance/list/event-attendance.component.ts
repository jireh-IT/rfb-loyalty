...
import { EntityArrayResponseType, EventAttendanceService } from '../service/event-attendance.service';
...
export class EventAttendanceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEventAttendance): number => this.eventAttendanceService.getEventAttendanceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.eventAttendances = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEventAttendance[] | null): IEventAttendance[] {
    // If there is previus link, data is a infinite scroll pagination content.
    if ('prev' in this.links) {
      const eventAttendancesNew = this.eventAttendances ?? [];
      if (data) {
        for (const d of data) {
          if (eventAttendancesNew.map(op => op.id).indexOf(d.id) === -1) {
            eventAttendancesNew.push(d);
          }
        }
      }
      return eventAttendancesNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.eventAttendanceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
