...
import { EntityArrayResponseType, RfbUserService } from '../service/rfb-user.service';
...
export class RfbUserComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRfbUser): number => this.rfbUserService.getRfbUserIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.rfbUsers = this.refineData(dataFromBody);
  }

  protected refineData(data: IRfbUser[]): IRfbUser[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IRfbUser[] | null): IRfbUser[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.rfbUserService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
